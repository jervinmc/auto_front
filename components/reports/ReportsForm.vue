      ...
      <div class="pa-5" align="start">
        <v-row>
          ...
          <v-col cols="12">
            <div>Comments</div>
            <div>
              <v-textarea v-model="reports.comments" outlined></v-textarea>
            </div>
          </v-col>
        </v-row>
        <div align="center">
          <v-btn depressed color="#6609af" dark @click="submit" :loading="buttonLoad"> Submit </v-btn>
        </div>
      </div>
    </v-card>
  </div>
  ...
  methods: {
     async submit(){
         ...
         var response = await this.$axios
          .post("/reports/", {"label":this.reports.comments,"user_name":`${localStorage.getItem('firstname')} ${localStorage.getItem('lastname')}`,"user_id":localStorage.getItem('id')},{
              headers: {
